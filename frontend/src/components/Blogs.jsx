...
import { FaThumbsUp, FaCommentDots, FaRegThumbsUp, FaRegCommentDots, FaUser, FaSortAmountDown, FaSortAmountDownAlt } from "react-icons/fa";
...
const Blogs = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      axios.get(`https://doubtful-waders-ray.cyclic.app/likedPost/${user._id}`).then((res) => setLikedBlogs(res.data));
    }
  }, [user]);
  ...
  const isLiked = (blogId) => likedBlogs.some((blog) => blog.post_id === blogId);
    ...
    if (user) {
      if (!isLiked(blog._id)) {
        try {
          ...
          axios.get(`https://doubtful-waders-ray.cyclic.app/likedPost/${user._id}`).then((res) => setLikedBlogs(res.data));
          axios.get(`https://doubtful-waders-ray.cyclic.app/blogs`).then((res) => setBlogs(res.data));
        } catch (error) {
    ...
    <section className="min-h-screen bg-gradient-to-b from-gray-100 to-gray-200 py-12">
      <div className="px-4 mx-auto max-w-screen-xl lg:px-6">
        <div className="mx-auto max-w-screen-md text-center mb-12">
          <h2 className="text-3xl font-extrabold text-gray-800 mb-4">Our Blog</h2>
          <p className="text-lg text-gray-600 mb-8">
            We use an agile approach to test assumptions and connect with the needs of your audience early and often.
          </p>
          {loading ? ( // Conditional rendering of SpinnerLoader component while loading
            <div className="flex items-center justify-center h-screen"> {/* Flex container to center SpinnerLoader */}
              <SpinnerLoader />
            </div>
          ):(<></>)}
          <div className="flex items-center justify-end mb-4">
            ...
            >
              ...
              {sortBlogsAsc ? <FaSortAmountDownAlt className="ml-2" /> : <FaSortAmountDown className="ml-2" />}
            </button>
          </div>
        </div>
                ...
                >
                  <div className="relative">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/60 to-transparent p-4">
                      ...
                      <p className="text-white text-sm">Posted: {formatDate(blog.createdAt)}</p>
                    </div>
                  </div>
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center">
                        ...
                        {!user ? (
                          <button className="flex items-center" onClick={handleUser}>
